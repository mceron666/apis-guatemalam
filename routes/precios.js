    ...
    try {
        ...
        const query = `
            SELECT 
                ...
                CASE (SELECT COUNT(*) FROM PAGOS_COLEGIATURA P INNER JOIN ALUMNOS_POR_GRADO A 
                ON P.ID_ALUMNO = A.ID_ALUMNO AND P.ID_PERIODO_ESCOLAR = A.ID_PERIODO_ESCOLAR 
                AND P.ID_PERIODO_ESCOLAR = 1 AND A.ID_GRADO = G.ID_GRADO )
				WHEN 0 THEN 'N' ELSE 'Y' END AS HAY_PAGOS                
